...
if __name__ == "__main__":
    ...
    parser.add_argument("-tr", "--train_argv", type=int, default=1, help="flag for toggling training")
...
#     model_dir, norm_range, minmax_file, dataset_spec, diff_spec, batch_num, lrs, batch_sizes, network_name, T, data_format = get_keys_from_json(json_config, keys=['model_dir','norm_range','minmax_file','dataset_spec', 'diff_spec', 'batch_num', 'lrs', 'batch_sizes', 'network_name', 'T', 'df'])
    dataset_spec, minmax_file, img_channels, data_format, norm_range, T, network_name, model_dir, n_delay, lr, batch_size = get_keys_from_json(json_config, keys=["dataset_spec", "minmax_file", "img_channels", "df", "norm_range", "T", "network_name", "model_dir", "n_delay", "learning_rate", "batch_size"])
#     lr = lrs[0]
#     batch_size = batch_sizes[0]
    ...
    for cr in cr_list:
        ...
        # train_ldr = torch.utils.data.DataLoader((torch.from_numpy(aux_train).to(device), torch.from_numpy(x_train).to(device)), batch_size=batch_size, shuffle=True) valid_ldr = torch.utils.data.DataLoader((torch.from_numpy(aux_val).to(device), torch.from_numpy(x_val).to(device)), batch_size=batch_size)
